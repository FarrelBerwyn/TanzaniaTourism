...
import { Users, Bed, Bath, ArrowUpRight, Check, Sparkles } from 'lucide-react';
...
export const VillasSection: React.FC<VillasSectionProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'ocean' | 'garden' | 'estate'>('all');

  const filteredVillas = VILLAS_DATA.filter((v) => {
    if (activeFilter === 'ocean') return v.id === 'villa-01' || v.id === 'villa-03' || v.id === 'villa-04';
    if (activeFilter === 'garden') return v.id === 'villa-02' || v.id === 'villa-05' || v.id === 'villa-07';
    if (activeFilter === 'estate') return v.id === 'villa-06' || v.id === 'villa-08';
    return true;
  });

  const featuredVilla = VILLAS_DATA[0]; // Villa 01 as primary flagship

  return (
    <section id="stay" className="py-24 md:py-32 bg-[#F4EFE6] text-[#1C1B1A]">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-[#E7DFD2]">
          <div>
            <div className="inline-flex items-center space-x-2 text-[11px] tracking-[0.3em] uppercase text-[#A07E54] font-medium mb-3">
              ...
              <span>Accommodation Portfolio</span>
            </div>
            <h2
              id="villas-heading"
          ...
          <div className="flex items-center space-x-2 mt-6 md:mt-0 overflow-x-auto pb-2">
            {[
              { key: 'all', label: 'All 8 Villas' },
              { key: 'ocean', label: 'Oceanfront' },
              { key: 'garden', label: 'Botanical Gardens' },
              { key: 'estate', label: 'Master Estates' },
            ].map((tab) => (
              <button
                key={tab.key}
        ...
        {/* Featured Flagship Villa Highlight (Villa 01) */}
        {activeFilter === 'all' && (
          <div className="mb-20">
            <div className="bg-[#FAF8F5] border border-[#E7DFD2] rounded-2xl overflow-hidden shadow-xl grid grid-cols-1 lg:grid-cols-12 gap-0 group">
              ...
              <div className="lg:col-span-7 relative aspect-[16/10] lg:aspect-auto overflow-hidden">
                ...
                <div className="absolute top-5 left-5 px-3.5 py-1.5 bg-[#141413]/85 backdrop-blur-md text-[#FAF8F5] text-xs tracking-[0.2em] uppercase font-semibold rounded flex items-center space-x-1.5">
                  ...
                  <span>{t.villas.featuredTag}</span>
                </div>
                <div className="absolute bottom-5 right-5 px-3 py-1 bg-black/70 backdrop-blur-md text-[#FAF8F5] text-sm font-serif rounded">
                  {featuredVilla.pricePerNight} <span className="text-[10px] font-sans text-[#D8CCB8]">{t.villas.perNight}</span>
                </div>
              </div>
              ...
              <div className="lg:col-span-5 p-8 sm:p-10 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="font-serif text-2xl sm:text-3xl font-normal text-[#141413] tracking-wide mb-2">
                    {featuredVilla.name}
                  </h3>
                  <p className="text-xs tracking-wider uppercase text-[#6B6862] font-medium mb-4">
                    {featuredVilla.type}
                  </p>
                ...
                {/* Actions */}
                <div className="flex items-center space-x-4 pt-4 border-t border-[#E7DFD2]">
                  <button
                    onClick={() => onSelectVilla(featuredVilla)}
                    className="flex-1 py-3 px-4 bg-[#1C1B1A] hover:bg-[#2C2B28] text-[#FAF8F5] text-xs font-semibold tracking-[0.16em] uppercase rounded flex items-center justify-center space-x-2 transition-colors"
                  >
                    <span>{t.villas.viewVilla}</span>
                    <ArrowUpRight className="w-4 h-4" />
                  </button>

                  <button
                    onClick={() => onRequestBooking(featuredVilla.id)}
                    className="py-3 px-5 border border-[#B8966C] hover:bg-[#B8966C]/15 text-[#8E6B40] text-xs font-semibold tracking-[0.16em] uppercase rounded transition-colors"
                  >
                    {t.villas.bookVilla}
                  </button>
                </div>
        ...
        {/* Asymmetric / Editorial Villa Grid for All 8 Units */}
        <div
          id="villas-grid"
            ...
            >
              ...
              <div className="relative aspect-[16/11] overflow-hidden">
                <img
                  ...
                  loading="lazy"
                />
                <div className="absolute top-4 left-4 px-2.5 py-1 bg-[#141413]/80 backdrop-blur text-[#FAF8F5] text-[11px] font-mono tracking-widest uppercase rounded">
                  {villa.roomNumber}
                </div>
                <div className="absolute bottom-3 right-3 px-2.5 py-1 bg-black/75 backdrop-blur text-[#FAF8F5] text-xs font-serif rounded">
                  {villa.pricePerNight} <span className="text-[10px] font-sans text-[#D8CCB8]">{t.villas.perNight}</span>
                </div>
              </div>
              ...
              <div className="p-6 flex flex-col flex-1 justify-between">
                <div>
                  <h4 className="font-serif text-xl font-normal text-[#141413] tracking-wide mb-1 group-hover:text-[#A07E54] transition-colors">
                    {villa.name}
                  </h4>
                  <p className="text-[11px] tracking-wider uppercase text-[#6B6862] font-medium mb-3">
                    {villa.type}
                  </p>
                  ...
                  {/* Micro Specs */}
                  <div className="flex items-center space-x-4 text-[11px] text-[#6B6862] py-3 border-t border-[#E7DFD2] mb-5">
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-transparent border border-[#1C1B1A]/25 hover:border-[#1C1B1A] hover:bg-[#1C1B1A] hover:text-[#FAF8F5] text-[#1C1B1A] text-xs font-semibold tracking-[0.16em] uppercase rounded flex items-center justify-center space-x-2 transition-all duration-300"
                >
                  <span>{t.villas.viewVilla}</span>
